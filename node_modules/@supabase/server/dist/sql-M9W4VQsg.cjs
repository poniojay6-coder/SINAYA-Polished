...
//#endregion
const require_errors = require("./errors-CwJBfRjf.cjs");
const require_error_response = require("./error-response-CeOZxFKf.cjs");
...
function createPostgresPool(pool, backoff) {
	...
	function acquireSlot() {
		...
		return new Promise((resolve, reject) => {
			...
			const timer = setTimeout(() => {
				...
				reject(require_errors.Errors[require_errors.PostgresPoolBusyError]({
					max,
	...
	async function connect() {
		...
		if (remaining > 0 && pool.idleCount <= pool.waitingCount) {
			releaseSlot();
			throw require_errors.Errors[require_errors.PostgresConnectPausedError]({
		...
		try {
			...
			if (pause > 0) console.error(`${LOG_PREFIX} connection failed, pausing new connections for ${pause}ms: ${require_errors.messageOf(e)}`);
			throw e;
		}
		let released = false;
		return {
...
Object.defineProperty(exports, "resolvePoolOptions", {
	enumerable: true,
	get: function() {
		return resolvePoolOptions;
	}
});
